...
function DashboardContent() {
    ...
    const handleCheckout = async () => {
        ...
        try {
            ...
            const message = `Hi! I just placed Order #${orderId.slice(-6).toUpperCase()}%0A%0A` +
                `Items:%0A${cartItems.map(item => `- ${item.quantity}x ${item.name} (₹${item.price * item.quantity})`).join('%0A')}%0A%0A` +
                `Total: ₹${cartTotal}`;
            ...
            // Switch to orders tab after a delay
            setTimeout(() => {
                setActiveTab("orders");
                                    ...
                                    <GlassCard className="border-purple-500/20">
                                        ...
                                        {orderSuccess ? (
                                            <div className="text-center py-4">
                                                ...
                                                <p className="text-stone-500 text-sm">Order #{orderSuccess.slice(-6).toUpperCase()}</p>
                                            </div>
                                        ) : (
                                            <button
                                                onClick={handleCheckout}
                                                disabled={checkingOut || !whatsappNumber}
                                                className="w-full inline-flex items-center justify-center gap-2 px-6 py-3 bg-green-600 text-white rounded-full text-sm font-medium hover:bg-green-500 transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
                                            >
                                                {checkingOut ? (
                                                    <>
